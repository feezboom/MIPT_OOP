...
//4)Сильно связными компонентами будут деревья поиска, построенные на шаге 3.
	...
	auto dfs_for_top(vertex<type>& vertex, fstream& log_file) {
		if (vertex.current_color == color::gray || vertex.current_color == color::black) {
			log_file << "tried to join vertex " << vertex.name << ", but failed : color == " << ((vertex.current_color == color :: gray) ? "gray" : "black") << endl;
			return;
		}
		 	
		log_file << "dfs timer == " << (vertex.time_in = timer++) << " joined " << vertex.name << endl;
		vertex.current_color = color::gray;

		for (auto& ptr : vertex.verteces_to) {//Проходимся по всем соседям вершины графа vertex, ptrs - итератор map<string, shared_ptr> соседей вершины
			dfs_for_top(*(ptr.second), log_file);
		}

		log_file << "dfs timer == " << (vertex.time_out = timer++)<< " left " << vertex.name << endl;
		vertex.current_color = color::black;
		return;
	}
	auto top_sort() {
		...
		auto& it = verteces.begin();
		fstream log_file;
		log_file.open("dfs_for_top.log", ios::trunc | ios_base::out);

		timer = 0;

		log_file << "dfs_for_top started from vertex " << it->second->name << endl;
		for (auto& it : verteces) { //Проходимся по всем вершинам графа, it - итератор map<string, shared_ptr<vertex>> вершин графа
			dfs_for_top(*(it.second), log_file);
		}
